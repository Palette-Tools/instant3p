...
const logoChalk = chalk.bold('instant3p-cli');
...
function warnDeprecation(oldCmd, newCmd) {
  warn(
    chalk.yellow('`instant3p-cli ' + oldCmd + '` is deprecated.') +
      ' Use ' +
      chalk.green('`instant3p-cli ' + newCmd + '`') +
      ' instead.' +
      '\n',
  );
}
...
/**
 * We need to do a bit of a hack of `@instantdb/react-native` and `@instant3p/electron`.
 *
 * If a user writes import { i } from '@instantdb/react-native' or '@instant3p/electron'
 *
 * We will fail to evaluate the file. This is because these packages bring in
 * native modules (react-native, electron) which do not run in a node context.
 *
 * To bypass this, we have a 'cli' module inside these packages, which
 * has all the necessary imports for CLI usage.
 */
function transformImports(code) {
  return code
    ...
    .replace(
      /"@instant3p\/electron"/g,
      '"@instant3p/electron/dist/cli"',
    );
}
...
async function readLocalPermsFile() {
  ...
  // Check if any perms files actually exist before calling unconfig
  // This prevents unconfig from creating temporary files when no configs exist
  ...
  for (const candidate of readCandidates) {
    ...
    for (const ext of extensions) {
      const filename = ext ? `${candidate.files}.${ext}` : candidate.files;
      try {
  ...
  if (!foundFile) {
    // No perms files exist, return early to avoid unconfig issues
    return;
  }
...
async function readLocalSchemaFile() {
  ...
  // Check if any schema files actually exist before calling unconfig
  // This prevents unconfig from creating temporary files when no configs exist
  const fs = await import('fs/promises');
  let foundFile = false;
  ...
  if (!foundFile) {
    // No schema files exist, return early to avoid unconfig issues
    return;
  }
...
async function readConfigAuthTokenWithErrorLogging() {
  ...
  if (!token) {
    error(
      `Looks like you are not logged in. Please log in with ${chalk.green('`instant3p-cli login`')}`,
    );
  }
  return token;
}
